...
int get_attr(uint16_t conn_handle,const struct ble_gatt_error *error,struct ble_gatt_attr *attr,void *arg){
  if(error->status == 0)
    printf("Got characteristic value %.*s\n", attr->om->om_len, attr->om->om_data);
  return error->status;
}
...
int servic_disc(uint16_t conn_handle,const struct ble_gatt_error *error,const struct ble_gatt_svc *service, void *arg){
...
int ble_gap_event(struct ble_gap_event *event, void *arg){
  ...
  case BLE_GAP_EVENT_CONNECT:
    ...
    if(event->connect.status == 0){
      ble_gattc_disc_all_svcs(event->connect.conn_handle, servic_disc, NULL);
    }
    break;
